...
export class HospitalAddSocialmediaComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.socialMediaForm.valid) {
      if (this.data.edit) {
        this.apiService
          .PutData(URLConstant.editSocialMedia, this.socialMediaForm.value, {
            userId: this.data.userId,
      ...
      this.apiService
        .Postdata(URLConstant.addSocialMedia, this.socialMediaForm.value, {
          userId: this.data.userId,
        })
        .subscribe({
          next: (res: any) => {
