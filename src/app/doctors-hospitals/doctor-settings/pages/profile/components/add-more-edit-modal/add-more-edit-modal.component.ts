...
export class AddMoreEditModalComponent implements OnInit {
  ...
  handleType7Submit() {
    ...
    const endpoint = this.matdata.edit
      ? URLConstant.addVideos + "?id=" + this.matdata.content.patchData._id
      : URLConstant.addVideos;
  ...
  submitData(endpoint: any, data: any, addFaq = false) {
    if (addFaq) {
      this.apiService.Postdata(endpoint, data, {}).subscribe((res: any) => {
        this.matdialogRef.close(true);
      });
    } else {
      this.apiService.PutData(endpoint, data, {}).subscribe((res: any) => {
        this.matdialogRef.close(true);
      });
    }
  }
  ...
  socialList() {
    this.apiService.GetData(URLConstant.social, {}).subscribe((res: any) => {
